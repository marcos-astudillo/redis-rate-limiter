...
export class BucketRepository {
   ...
   */
  private async execScript(keys: string[], args: number[]): Promise<number[]> {
    const strArgs = args.map(String);

    if (!this.sha) {
      this.sha = (await this.redis.call('SCRIPT', 'LOAD', TOKEN_BUCKET_SCRIPT)) as string;
      logger.debug('BucketRepository', 'Lua script loaded', { sha: this.sha });
    }
    ...
    try {
      ...
      if (msg.includes('NOSCRIPT')) {
        // Script was evicted from Redis — reload and retry once
        this.sha = null;
        return this.execScript(keys, args);
      }
      throw err;
    }
  }
